...
// Trend and popular watches created
const popularWatchesContainer = document.querySelector('.popural-watch__container');
const trendWatchesContainer = document.querySelector('.trend-watch__container');

const trendAndWatches = [
  {
    id: 1,
...
const generateWatchCard = (watch) => `
  <div class="p-2 border-r border-gray-300">
    ...
    <div class="flex justify-center items-center text-center gap-x-2 mt-4 mb-4">
      <div>
        <button class="flex gap-x-1 border border-zinc-950 p-2 group hover:bg-zinc-950">
          <svg class="w-5 h-5 text-zinc-950 group-hover:text-white">
            <use href="#shopping-card"></use>
          </svg>
          <span class="text-zinc-950 font-VazirmatnRegular group-hover:text-white">
            اضافه کردن به سبد
          </span>
        </button>
      </div>
      <div>
        <button class="border border-zinc-950 p-2.5 group hover:bg-zinc-950">
...
const renderWatches = (container, watches) => {
  watches.forEach((watch) => {
    container.insertAdjacentHTML('beforeend', generateWatchCard(watch));
  });
};

renderWatches(trendWatchesContainer, trendAndWatches);
renderWatches(popularWatchesContainer, trendAndWatches);
